...
def bai_tap_hoc_vien(request, lop_id):
    ...
    ds_bai_tap = BaiTap.objects.filter(lop_hoc=lop)
    bai_da_nop = {n.bai_tap_id: n for n in NopBaiTap.objects.filter(hoc_vien=hv, bai_tap__lop_hoc=lop)}
    return render(request, 'bai_tap_hoc_vien.html', {
        'lop': lop, 'ds_bai_tap': ds_bai_tap, 'bai_da_nop': bai_da_nop
    })
...
def thoi_gian_ranh_gv(request):
    ...
    return render(request, 'thoi_gian_ranh.html', {
        'giao_vien': gv, 'ds_ranh': ds_ranh, 'ranh_set': ranh_set,
        'thu_range': range(2, 9), 'ca_range': range(1, 7)
    })
...
def xem_lich_bu(request):
    ...
    da_dang_ky = {}
    if hv:
        da_dang_ky = {dk.lich_bu_id: dk for dk in DangKyLichBu.objects.filter(hoc_vien=hv)}

    return render(request, 'xem_lich_bu.html', {
        'ds_lb': ds_lb, 'is_admin': is_admin, 'giao_vien': gv,
        'hoc_vien': hv, 'da_dang_ky': da_dang_ky
    })
