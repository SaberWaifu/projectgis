...
class GioiThieu(models.Model):
    ...
    tom_tat = models.TextField(error_messages=LOI_CHUOI)
    noi_dung = models.TextField(error_messages=LOI_CHUOI)
    loai_muc = models.CharField(max_length=20, choices=LOAI_MUC, default='TONG_QUAN')
...
class AnhGioiThieu(models.Model):
    gioi_thieu = models.ForeignKey(GioiThieu, on_delete=models.CASCADE, related_name='cac_anh')
    hinh_anh = models.ImageField(upload_to='gioi_thieu/', max_length=500)

    def __str__(self):
        return f"Ảnh #{self.id} - {self.gioi_thieu.tieu_de}"
